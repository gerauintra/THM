...
find_files() {
    find /opt/THM/Anthem -type f \( -name "*.md" -o -name "*.sh" \)
}
...
change_IP() {
    ...
    echo "IP address changed from $old_ip to $new_ip in all files in directory /opt/THM/Anthem"
}
...
change_HOST() {
    ...
    echo "Hostname changed from $old_host to $new_host in all files in directory /opt/THM/Anthem"
}
...
change_INTERFACE() {
    ...
    echo "Network interface changed from $old_inf to $new_inf in all files in directory /opt/THM/Anthem"
}

main() {
    if [ ! -z "10.10.206.242" ]; then
        change_IP
    fi
    if [ ! -z "anthem.thm" ]; then
        change_HOST
    fi
    if [ ! -z "tun0" ]; then
        change_INTERFACE
    ...
    if [ -z "10.10.206.242" ] && [ -z "anthem.thm" ] && [ -z "tun0" ]; then
        usage
    fi
}
